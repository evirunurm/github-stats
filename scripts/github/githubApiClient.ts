...
import { MOCK_RESPONSE } from "./__mock__/mockGithubApiResponse";
...
const makeGraphQLRequest = (query: string): Promise<GitHubApiResponse> => {
  ...
  return new Promise<GitHubApiResponse>((resolve, reject) => {
    const req = https.request(options, (res) => {
      ...
      res.on("end", () => {
        try {
          resolve(JSON.parse(Buffer.concat(buffers).toString()) as GitHubApiResponse);
        } catch (e) {
          reject(new Error(`Failed to parse GitHub API response: ${(e as Error).message}`));
        }
      });
    });
